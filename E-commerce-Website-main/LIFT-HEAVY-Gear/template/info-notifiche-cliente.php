<section>
    ...
    <?php if (count($templateParams["notifiche"]) > 0) : ?>
        <?php foreach ($templateParams["notifiche"] as $notifica) : ?>
            <div class="notifica" id="notifica-<?php echo $notifica["ID_notifica"]; ?>">
                ...
                <form method="POST" action="notifiche.php" enctype="multipart/form-data">
                    <input type="hidden" name="action" value="1" />
                    <label for="letto-<?php echo $notifica["ID_notifica"]; ?>" class="checkbox-label">
                        <span>Contrassegna come letto: </span>
                        <input type="checkbox" id="letto-<?php echo $notifica["ID_notifica"]; ?>" value="letto" name="letto" <?php if ($notifica["stato_notifica"] == "Letta") echo "checked"; ?> />
                    </label>
                    <input type="hidden" name="notifica_id" value="<?php echo $notifica["ID_notifica"]; ?>">
                    <button type="submit" name="salva_notifica" class="salva">Salva modifica</button>
                </form>
